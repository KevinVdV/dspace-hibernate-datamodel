...
implements DOIConnector
{
    ...
    public boolean isDOIReserved(Context context, DSpaceObject dso, String doi)
        ...
        switch (resp.getStatusCode())
        {
            ...
            case (200) :
            {
                ...
                String dsoHandle = dso.getHandle(context);
                if (null == dsoHandle)
                {
                    return false;
                }
            ...
            throws DOIIdentifierException
    {
        ...
        catch (URISyntaxException e)
        {
            ...
            log.error("The URL was {}.", SCHEME + "://" + HOST +
                    DOI_PATH + "/" + doi.substring(DOIService.SCHEME.length()));
        ...
        try
        {
            String req = "doi=" + doi.substring(DOIService.SCHEME.length()) + "\n" + "url=" + url + "\n";
    ...
    protected Element addDOI(String doi, Element root) {
        ...
        identifier.addContent(doi.substring(DOIService.SCHEME.length()));
