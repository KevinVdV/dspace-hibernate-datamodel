 ...
 * @version $Revision$
 */
public class BitstreamFormatDAO
{
    /** log4j logger */
     ...
     * @throws SQLException
     */
    public BitstreamFormat[] findAll(Context context)
            throws SQLException
    {
        Criteria criteria = context.getDBConnection().createCriteria(BitstreamFormat.class);
        criteria.addOrder(Order.asc("bitstream_format_id"));
        List<BitstreamFormat> formats = criteria.list();
        return formats.toArray(new BitstreamFormat[formats.size()]);
    }
            ...
            AuthorizeException
    {
        ...
        // Create a table row
        BitstreamFormat bitstreamFormat = new BitstreamFormat();
        HibernateQueryUtil.update(context, bitstreamFormat);
    ...
    public void delete(Context context, BitstreamFormat bitstreamFormat) throws SQLException, AuthorizeException
    {
        ...
        Query query = context.getDBConnection().createQuery("update bitstream set bitstream_format_id = :unknown_format_id where bitstream_format_id = :deleted_format_id");
        query.setParameter("unknown_format_id", unknown.getID());
        query.setParameter("deleted_format_id", bitstreamFormat.getID());

        // Set bitstreams with this format to "unknown"
        int numberChanged = query.executeUpdate();

        // Delete this format from database
        HibernateQueryUtil.delete(context, bitstreamFormat);
