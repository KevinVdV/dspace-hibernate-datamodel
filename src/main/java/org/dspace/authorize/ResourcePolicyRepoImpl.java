 ...
 * @version $Revision$
 */
public class ResourcePolicyRepoImpl implements ResourcePolicyRepo
{
    /** log4j logger */
    private static Logger log = Logger.getLogger(ResourcePolicyRepoImpl.class);
    ...
    public void delete(Context context, ResourcePolicy resourcePolicy) throws SQLException
    {
        if(resourcePolicy.getResourceID() != -1 && resourcePolicy.getResourceType() != -1)
        {
            ...
            DSpaceObject dso = DSpaceObjectRepoImpl.find(context, resourcePolicy.getResourceType(), resourcePolicy.getResourceID());
            if(dso != null)
            {
