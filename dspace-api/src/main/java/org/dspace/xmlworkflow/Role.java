...
public class Role {
    ...
    public RoleMembers getMembers(Context context, XmlWorkflowItem wfi) throws SQLException {
        ...
        if(scope == Scope.COLLECTION){
            CollectionRole collectionRole = collectionRoleService.findByCollectionAndRole(context, wfi.getCollection(), id);
            ...
            List<WorkflowItemRole> roles = workflowItemRoleService.findByWorkflowItemAndRole(context, wfi, id);
            RoleMembers assignees = new RoleMembers();
