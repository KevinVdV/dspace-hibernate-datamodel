...
public class XmlWorkflowServiceImpl implements XmlWorkflowService {
    ...
    public XmlWorkflowItem start(Context context, WorkspaceItem wsi) throws SQLException, AuthorizeException, IOException, WorkflowException {
        try {
            ...
            Workflow wf = workflowFactory.getWorkflow(context, collection);
    ...
    @Override
    public void alertUsersOnTaskActivation(Context c, XmlWorkflowItem wfi, String emailTemplate, List<EPerson> epa, String ...arguments) throws IOException, SQLException {
        if (noEMail.containsKey(wfi.getItem().getID())) {
            ...
            try {
                ...
                log.warn(LogManager.getHeader(c, "notifyOfTask",
                        "cannot email users" + " item_id=" + wfi.getItem().getID()));
            }
        }
    }
    ...
    protected WorkflowActionConfig processOutcome(Context c, EPerson user, Workflow workflow, Step currentStep, WorkflowActionConfig currentActionConfig, ActionResult currentOutcome, XmlWorkflowItem wfi, boolean enteredNewStep) throws IOException, WorkflowConfigurationException, AuthorizeException, SQLException, WorkflowException {
        ...
        if (currentOutcome.getType() == ActionResult.TYPE.TYPE_OUTCOME) {
            ...
            try {
                ...
                if(enteredNewStep){
                    ...
                    ClaimedTask task = claimedTaskService.findByWorkflowItemAndEPerson(c, wfi, user);
    ...
    protected void logWorkflowEvent(Context c, String workflowId, String previousStepId, String previousActionConfigId, XmlWorkflowItem wfi, EPerson actor, Step newStep, WorkflowActionConfig newActionConfig) throws SQLException {
        try {
            ...
            if(newStep != null && newActionConfig != null){
                ...
                List<ClaimedTask> claimedTasks = claimedTaskService.findByWorkflowItemAndStepId(c, wfi, newStep.getId());
                List<PoolTask> pooledTasks = poolTaskService.findByWorkflowItem(c, wfi);
...
//                nextActionConfig.getProcessingAction().generateTasks();
     ...
     */
    @Override
    public Item archive(Context c, XmlWorkflowItem wfi)
        ...
        itemService.clearMetadata(c, item, WorkflowRequirementsServiceImpl.WORKFLOW_SCHEMA, Item.ANY, Item.ANY, Item.ANY);
    ...
    public void deleteAllTasks(Context c, XmlWorkflowItem wi) throws SQLException, AuthorizeException {
        ...
        Iterator<ClaimedTask> allClaimedTasks = claimedTaskService.findByWorkflowItem(c,wi).iterator();
    ...
    public void deleteAllPooledTasks(Context c, XmlWorkflowItem wi) throws SQLException, AuthorizeException {
        Iterator<PoolTask> allPooledTasks = poolTaskService.findByWorkflowItem(c, wi).iterator();
    ...
    public void deletePooledTask(Context c, XmlWorkflowItem wi, PoolTask task) throws SQLException, AuthorizeException {
        if(task != null){
            poolTaskService.delete(c, task);
            if(task.getePerson() != null){
                removeUserItemPolicies(c, wi.getItem(), task.getePerson());
            }else{
                removeGroupItemPolicies(c, wi.getItem(), task.getGroup());
            }
        }
    }
    ...
    public void createOwnedTask(Context c, XmlWorkflowItem wi, Step step, WorkflowActionConfig action, EPerson e) throws SQLException, AuthorizeException {
        ClaimedTask task = claimedTaskService.create(c);
        task.setWorkflowItem(wi);
        task.setStepId(step.getId());
        task.setActionId(action.getId());
        task.setOwner(e);
        task.setWorkflowId(step.getWorkflow().getID());
        claimedTaskService.update(c, task);
        //Make sure this user has a task
        ...
        if(epa != null){
            ...
            for (Bundle bundle : bundles) {
                ...
                for (BundleBitstream bundleBitstream : bits) {
                    authorizeService.addPolicy(context, bundleBitstream.getBitstream(), type, epa);
                }
            }
        }
    }
    protected void addGroupPolicyToItem(Context context, Item item, int type, Group group) throws AuthorizeException, SQLException {
        if(group != null){
            ...
            for (Bundle bundle : bundles) {
                ...
                List<BundleBitstream> bits = bundle.getBitstreams();
                for (BundleBitstream bundleBitstream : bits) {
                    authorizeService.addPolicy(context, bundleBitstream.getBitstream(), type, group);
                }
            }
        }
    }
        ...
        if(e != null){
            ...
            for (Bundle bundle : bundles) {
                ...
                for (BundleBitstream bundleBitstream : bitstreams) {
                    authorizeService.removeEPersonPolicies(context, bundleBitstream.getBitstream(), e);
                }
            }
    ...
    * @param rejection_message
     ...
     */
    @Override
    public WorkspaceItem sendWorkflowItemBackSubmission(Context c, XmlWorkflowItem wi, EPerson e, String provenance,
            String rejection_message) throws SQLException, AuthorizeException, IOException
    {

        String workflowID = null;
        String currentStepId = null;
        String currentActionConfigId = null;
        ClaimedTask claimedTask = claimedTaskService.findByWorkflowItemAndEPerson(c, wi, e);
        if(claimedTask != null){
            //Log it
        ...
        itemService.clearMetadata(c, myItem, WorkflowRequirementsServiceImpl.WORKFLOW_SCHEMA, Item.ANY, Item.ANY, Item.ANY);

        itemService.update(c, myItem);

        WorkspaceItem wsi = sendBackToWorkspace(c, wi, e);
    ...
    public WorkspaceItem abort(Context c, XmlWorkflowItem wi, EPerson e) throws SQLException, AuthorizeException, IOException {
        ...
        return sendBackToWorkspace(c, wi, e);
    }

    protected WorkspaceItem sendBackToWorkspace(Context c, XmlWorkflowItem wi, EPerson e) throws SQLException, AuthorizeException, IOException {
        ...
        // Remove (if any) the workflowItemroles for this item
        workflowItemRoleService.deleteForWorkflowItem(c, wi);
