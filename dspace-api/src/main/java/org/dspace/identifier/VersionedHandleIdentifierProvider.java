...
public class VersionedHandleIdentifierProvider extends IdentifierProvider {
        ...
        try
        {
            ...
            if(dso != null && dso.getType() == Constants.ITEM)
            {
                ...
                if(history!=null)
                {
                    ...
                    Version version = versionService.findByItem(context, item);
    ...
    private VersionHistory getHistory(Context context, String identifier) throws SQLException {
    ...
    private void restoreItAsVersion(Context context, DSpaceObject dso, String identifier, Item item, String canonical, VersionHistory history) throws SQLException, IOException, AuthorizeException
    {
        ...
        versionService.createVersion(context, history, item, "Restoring from AIP Service", new Date(), versionNumber);
     ...
     */
    public String mint(Context context, DSpaceObject dso) throws SQLException {
        if(dso.getHandle(context) != null)
        {
            return dso.getHandle(context);
        }
    ...
    protected String getCanonical(Context context, Item item) throws SQLException {
        String canonical = item.getHandle(context);
