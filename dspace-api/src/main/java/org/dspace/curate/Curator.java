...
public class Curator
{
    ...
    private boolean doSite(TaskRunner tr, Site site) throws IOException
    {
        ...
        try
        {
            ...
            if (! tr.run(curationContext(), site))
            {
                return false;
            }
    ...
    private boolean doCommunity(TaskRunner tr, Community comm) throws IOException
    {
        try
        {
        if (! tr.run(curationContext(), comm))
        {
            return false;
        }
    ...
    private boolean doCollection(TaskRunner tr, Collection coll) throws IOException
    {
        try
        {
            if (! tr.run(curationContext(), coll))
            {
                return false;
            }
    ...
    private class TaskRunner
    {
        ...
        public boolean run(Context context, DSpaceObject dso) throws IOException {
            ...
            {    
                ...
                String id = (dso.getHandle(context) != null) ? dso.getHandle(context) : "workflow item: " + dso.getID();
            ...
            catch(IOException ioe)
            {
                ...
                log.error("Error executing curation task '" + task.getName() + "'", sqlE);
                throw new IOException(sqlE.getMessage(), sqlE);
